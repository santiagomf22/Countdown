
export const partidos = [
    {
      partido: "NIVELACIÓN DE LA MESA",
      texto1: "Total sufragantes formato E-11",
      texto2: "Total votos en la urna",
    },
    {
      partido: "COALICIÓN PACTO HISTÓRICO",
      oid:"1",
      imagen: require("../../assets/pacto_historico.png"),
      candidatos: ["Solo partido"],
    },
    {
      partido: "PARTIDO POLITICO DIGNIDAD",
      oid:"2",
      imagen: require("../../assets/dignidad.png"),
      candidatos: ["Solo partido", "101", "102", "103", "104"],
    },
    {
      partido: "COALICIÓN PARTIDOS CONSERVADOR COLOMBIANO - COLOMBIA JUSTA Y LIBRES - PARTIDO SALVACIÓN NACIONAL",
      oid:"3",
      imagen: require("../../assets/conservador.png"),
      candidatos: ["Solo partido", "101", "102", "103", "104"],
    },
    {
      partido: 'PARTIDO ALIANZA SOCIAL INDEPENDIENTE "ASI"',
      oid:"4",
      imagen: require("../../assets/asi.png"),
      candidatos: ["Solo partido", "101", "102", "103", "104"],
    },
    {
      partido: "PARTIDO LIBERAL COLOMBIANO",
      oid:"5",
      imagen: require("../../assets/liberal.png"),
      candidatos: ["Solo partido", "101", "102", "103", "104"],
    },
    {
      partido: "COALICIÓN PARTIDO CAMBIO RADICAL Y PARTIDO POLITICO MIRA",
      oid:"6",
      imagen: require("../../assets/cambio_radical.png"),
      candidatos: ["Solo partido", "101", "102", "103", "104"],
    },
    {
      partido: 'PARTIDO DE LA UNIÓN POR LA GENTE "PARTIDO DE LA U"',
      oid:"7",
      imagen: require("../../assets/u.png"),
      candidatos: ["Solo partido", "101", "102", "103", "104"],
    },
    {
      partido: "VOTOS NULOS",
      oid:"8",
      imagen: require("../../assets/voto_nulo.png"),
      candidatos: ["Votos"],
    },
    {
      partido: "VOTOS EN BLANCO",
      oid:"9",
      imagen: require("../../assets/voto_blanco.png"),
      candidatos: ["Votos"],
    },
    {
      partido: "VOTOS NO MARCADOS",
      oid:"10",
      imagen: require("../../assets/voto_no_marcado.png"),
      candidatos: ["Votos"],
    },
  ];
